...
public class ClientRegistryEvents {
    ...
    public static void registerItemColor(RegisterColorHandlersEvent.Item event) {
        for(Map.Entry<PigmentColor, PigmentBlockSet> blockSet : ModBlocks.pigmentBlocks.entrySet()) {
            ...
            registerItemPigmentColor(event, blockSet.getValue().StainedGlass.asItem(), blockSet.getKey(), 1.0f);
            registerItemPigmentColor(event, blockSet.getValue().StainedGlassPane.asItem(), blockSet.getKey(), 1.0f);
        }
    }
    ...
    public static void registerBlockColor(RegisterColorHandlersEvent.Block event) {
        for(Map.Entry<PigmentColor, PigmentBlockSet> blockSet : ModBlocks.pigmentBlocks.entrySet()) {
            ...
            registerBlockPigmentColor(event, blockSet.getValue().StainedGlass.get(), blockSet.getKey(), 1.0f);
            registerBlockPigmentColor(event, blockSet.getValue().StainedGlassPane.get(), blockSet.getKey(), 1.0f);
        }
    }
    ...
    public static void registerBlockPigmentColor(RegisterColorHandlersEvent.Block event, Block block, PigmentColor pigmentColor, float multiplier) {
        if(pigmentColor != PigmentColor.EGGSHELL) {
